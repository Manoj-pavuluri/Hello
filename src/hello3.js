...
class InpCom extends Component {
    constructor() {
        super();
        this.state = {
            autoFocus: true,
    ...
    getInput = () => {
        ...
        return self.state.data.map((val, ind) => {
            return (
                < div key={ind}>
                    <span> <input  onChange={(event) => self.changeVal(event, ind, 'name')} value={val.name} type='text' /></span>
                    <span> <input  value={val.id} onChange={(event) => { self.changeVal(event, ind, 'id') }} type='text' /></span>
                    <br />
                </div>
            )
        })
